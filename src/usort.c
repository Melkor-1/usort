...
static void parse_options (const struct option long_options[],
                             struct flags *opt_ptr, int argc, char *argv[])
{
    int c;

    while ((c =
            getopt_long (argc, argv, OP_LIST, long_options, NULL)) != -1) {
        switch (c) {
            case 'n':
                opt_ptr->nflag = true;
                break;
...
*/
static bool write_line (char *lines[], size_t num_lines,
...
static int case_cmp (const void *s1, const void *s2) 
{
...
static int s_cmp (const void *s1, const void *s2)
{
    const char *const *v1 = s1;
	const char *const *v2 = s2;
...
static int num_cmp (const void *s1, const void *s2)
{
...
static char *read_file (FILE * fp)
{
    ...
    for (size_t rcount = 1; rcount; len += rcount) {
        char *tmp = realloc (content, len + page_size);
...
static size_t split_lines (char *s, char ***lines)
{
...
int main (int argc, char *argv[])
{
    ...
    /* Note regarding memory leak: 
     *  read_file returns NULL on failure, in which case it freed the content pointer. Then the 
     *  content pointer to split_lines, which checks whether content is NULL and 
     *  returns execution back to the main. After which, content is freed (which was 
     *  already freed, but initialised to NULL, so it has no effect), but do not free the lines pointer.
     *  But the lines pointer was allocated on the stack, so it need not be freed. Whereas content
     *  on success would have been allocated, passed to read_lines for parsing, remained alocated
     *  on the heap, and in need of being returned. So we free it and there remains no memory
     *  leaks in the code.
     */
	...
	qsort (lines, num_lines, sizeof *lines, options.nflag ? num_cmp : options.fflag ? case_cmp : s_cmp);
